...
const authRouter = ({ provider, adapter, jwtToken }) => {
  const CLIENT_HOME_URL = `${process.env.CLIENT_URL?.slice(1, -1)}/#/login`;
  ...
  passport.use(
    new JwtStrategy(
      ...
      async (jwt_payload, done) => {
        ...
        (await adapter.find(jwt_payload.data))
  ...
  if (provider.GITHUB) {
    ...
    AuthRouter.get(
      ...
      async (req, res) => {
        // console.log("from github", { user: req.user });
        const user = {
  ...
  AuthRouter.get("/refresh", (req, res) => {
    ...
    jwt.verify(refreshToken, jwtToken.refreshToken, async (err, decoded) => {
